...
在 Cloudflare 控制台的 `Settings → Variables` 中添加 `DNS_UPSTREAMS_` 前缀的变量，一条变量 = 一个上游，值为完整的 DoH URL：

| 变量名 | 变量值 |
|---|---|
| `DNS_UPSTREAMS_google` | `https://dns.google/dns-query` |
| `DNS_UPSTREAMS_cloudflare` | `https://cloudflare-dns.com/dns-query` |
| `DNS_UPSTREAMS_quad9` | `https://dns.quad9.net/dns-query` |
| `DNS_UPSTREAMS_alidns` | `https://dns.alidns.com/dns-query` |

设置后，对应的访问路径为 `/<变量名后缀>/query-dns`，例如 `/google/query-dns`、`/quad9/query-dns`。添加/删除上游就是加一条/删一条变量，无需 JSON。
